...
export class UserService {
  ...
  getUser(userId: number | undefined): Observable<User> {
    return this.http.get<User>("http://localhost:2020/api/users/getUser/" + userId);
  }
  ...
  updateEmployeeInfo(userModel: User): Observable<GenericResponse> {
    return this.http.put<GenericResponse>("http://localhost:2020/api/users/update", userModel);
  }
